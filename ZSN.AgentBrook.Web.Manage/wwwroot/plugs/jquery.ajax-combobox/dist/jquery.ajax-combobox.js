 ...
 * @param {boolean} [option.instance] - 实例
 * @param {string} [option.lang='ja'] - 语言。 ('ja', 'en', 'es' and 'pt-br')
 * @param {string} [option.db_table='tbl'] - 反问数据表名
 * @param {string} [option.field='name'] - 返回数据结果集中的字段名
 * @param {string} [option.search_field=option.field] - 检索字段名 (e.g.: 'id, name, job')
 * @param {string|Array} [option.order_by=option.search_field] - 排序 (e.g.: 'name DESC' or ['name ASC', 'age DESC'])
 * @param {number} [option.per_page=10] - 分页条数。
 * @param {number} [option.navi_num=5] - 当前页码。
 * @param {boolean} [option.navi_simple=false] - 是否显示导航信息。
 * @param {string} [option.plugin_type='combobox'] - 'combobox', 'simple', 'textarea'
 * @param {string} [option.init_record=false] - 初始化指定记录。 
 * @param {string} [option.bind_to] - 选中后绑定事件
 * @param {string} [option.and_or='AND'] - 检索方式 ('AND' or 'OR')
 * @param {boolean|string} [option.sub_info=false] - 子信息。 (true, false or 'simple')
 ...
 * @param {string} [option.primary_key='id'] - 
 * @param {string} [option.button_img='dist/btn.png'] - 
 * @param {string} [option.shorten_btn] - (option.plugin_type='textarea' 短按钮。
 * @param {string} [option.shorten_src='dist/bitly.php'] - 短弟子。
 * @param {number} [option.shorten_min=20] - 缩短文字数。
 * @param {Object} [option.shorten_reg] - URL监测。
 ...
 * @global
 * @constructor
 * @classdesc 收集对每个元素应用的处理的类
 * @param {Object} combo_input - 应用插件的HTML元素。
 * @param {string|Object} source - 服务器端的程序或者联想排列本身。
 * @param {Object} option - 包含选项的联想排列。
 */
...
$.extend(AjaxComboBox.prototype, /** @lends AjaxComboBox.prototype */ {
   ...
   * @private
    * @desc 初始化选项
   * @param {string|Object} source - 通往服务器语言的路径或数据本身的联想排列
   * @param {Object} option - 联想数组形式的选项
   */
    ...
    switch (this.option.lang) {
      ...
      default:
         message = {
          ...
          del_title   : '删除标题',
          next        : '下页',
          next_title  : '下' + this.option.per_page + ' (右键)',
          prev        : '上页',
          prev_title  : '上' + this.option.per_page + ' (左键)',
          first_title : '第一页',
          last_title  : '最后一页',
          get_all_btn : '所有',
          get_all_alt : '按钮',
          ...
          loading     : 'loading...',
          loading_alt : '(loading)',
          page_info   : 'num_page_top - num_page_end',
          select_ng   : 'Attention : Please choose from among the list.',
          select_ok   : 'OK',
          not_found   : '没有数据',
          ajax_error  : '错误连接服务器失败'
        };
    }
  ...
  _setElem: function(combo_input) {
    ...
    if (this.option.plugin_type == 'combobox') {
        ...
        //$(elem.container).width($(elem.combo_input).width() + $(elem.button).width()+10);
        
    } else {
      //$(elem.container).width($(elem.combo_input).width()+10);
    }
  ...
  _searchForDb: function(self, q_word, which_page_num) {
    self.prop.xhr = $.ajax({
      ...
      success: function(json) {
        ...
        json.cnt_page = json.results.length;
        for (i = 0; i < json.cnt_page; i++) {
          json.subinfo[i] = [];
          for (var key in json.results[i]) {
            if (key == self.option.primary_key) {
              json.primary_key.push(json.results[i][key]);
            }
            if (key == self.option.field) {
              json.candidate.push(json.results[i][key]);
              ...
              if (
                ...
                json.subinfo[i][key] = json.results[i][key];
              }
            }
          }
        }
        //delete(json.results);//cxty
  ...
  _prepareResults: function(self, json, q_word, which_page_num) {
    ...
    // 候補リストを表示する//cxty
    self._displayResults(self, json.candidate, json.subinfo, json.primary_key, json.results);
   ...
   */
  _displayResults: function(self, arr_candidate, arr_subinfo, arr_primary_key, _c_data) {
    // 候補リストを、一旦リセット
    $(self.elem.results).empty();
    $(self.elem.sub_info).empty();
  ...
  _selectCurrentLine: function(self, is_enter_key) {
    ...
    if (self.option.bind_to) {
      // 候補選択を引き金に、イベントを発火する
      $(self.elem.combo_input).trigger(self.option.bind_to, [is_enter_key,_data]);
    }
